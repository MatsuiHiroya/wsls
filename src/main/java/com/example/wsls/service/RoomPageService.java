...
public class RoomPageService implements IRoomPageService {
    ...
    public List<PostedRandom> calculation(Integer roomId){
        ...
        postedRandomList.stream()
                    ...
                    .forEach(item -> {
                        ...
                        Long randoms = item.getRandom() + sponsorRandom + totalRandom;
                        //h(ri,rs,sum(r1...rn,rs))を計算
                        var hashedRandom = randomToHash(randoms);
        ...
        var sorted = lotteryResultList.stream()
                .sorted(Comparator.comparing(PostedRandom::getResult))
    ...
    private Long hashToLong(String hashResult) {
        ...
        System.out.println("抽選数値:" + (multipleTotal + sumTotal));
        return multipleTotal + sumTotal;

    }

    @Override
    public void closeLottery(Integer roomId){
        int n = roomPageRepository.closeLottery(roomId);
        System.out.println("update:" + n);
    }

    @Override
    public PostedRandom findSponsorInfo(Integer roomId,Integer sponsorId){
        return roomPageRepository.findSponsorInfo(roomId,sponsorId);
    }
